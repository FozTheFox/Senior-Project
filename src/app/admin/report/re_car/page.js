...
const fetchQuotationInvoice = async () => {
  const response = await fetch("https://bodyworkandpaint.pantook.com/api/Quotationinvoice");
...
const Report = () => {
  ...
  const [repairProcesses, setRepairProcesses] = useState(null); // สร้าง state สำหรับ repairProcesses
  ...
  const fetchRepairProcesses = async (quotationId) => {
    ...
    const selectedData = data.data.find(item => item.Quotation_ID === quotationId);
    setRepairProcesses(selectedData?.repairProcesses || null);
  };

  useEffect(() => {
    if (selectedQuotation) {
      fetchRepairProcesses(selectedQuotation.Quotation_ID); // ดึง repairProcesses
    }
  }, [selectedQuotation]);
  ...
  const filteredQuotations = useMemo(() => {
    // ตรวจสอบว่า quotationsData มีโครงสร้างที่คาดหวัง
  ...
  const generatePrintPage = async () => {
    ...
    const selectedAdditionalData = additionalData.find(item => item.Quotation_ID === selectedQuotation.Quotation_ID);
